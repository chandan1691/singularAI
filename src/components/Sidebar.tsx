            ...
            <div className="p-6 pb-4 shrink-0">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className={isDarkMode ? 'text-white' : 'text-gray-900'}>Ada AI</h2>
                    <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>V1.0</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => setIsDarkMode(!isDarkMode)}
                    className={`relative flex items-center gap-1 p-1 rounded-full transition-all ${
                      isDarkMode 
                        ? 'bg-gray-800' 
                        : 'bg-gray-900'
                    }`}
                  ...
                  >
                    <div 
                      className={`absolute top-1 left-1 w-8 h-8 bg-white rounded-full transition-transform duration-300 ease-in-out ${
              ...
              <div className="relative">
                <Search className={`w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 ${isDarkMode ? 'text-gray-500' : 'text-gray-400'} pointer-events-none`} />
                <Input
                  ref={searchInputRef}
                  type="text"
                  ...
                  className={`pl-9 backdrop-blur-sm rounded-full ${
                    isDarkMode 
                      ...
                      : 'bg-white/60 border-white/80 text-gray-900'
                  }`}
                />
                ...
                {isSearchFocused && !searchQuery && (
                  <div className={`absolute top-full left-0 right-0 mt-2 p-3 rounded-2xl shadow-lg backdrop-blur-xl z-10 ${
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Recent searches</span>
                    </div>
                            ...
                            <TooltipProvider>
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <span className="flex-1 text-sm truncate overflow-hidden whitespace-nowrap">{category.name}</span>
                                </TooltipTrigger>
                                <TooltipContent>
                                  <p>{category.name}</p>
                                </TooltipContent>
